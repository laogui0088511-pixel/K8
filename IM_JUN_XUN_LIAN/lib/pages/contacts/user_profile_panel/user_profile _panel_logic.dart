...
class UserProfilePanelLogic extends GetxController {
    ...
    try {
      Apis.queryUserOnlineStatus(
        ...
        onlineStatusCallback: (map) {
          onlineStatus.value = map[userInfo.value.userID!] ?? false;
        },
        onlineStatusDescCallback: (map) {
          onlineStatusDesc.value = map[userInfo.value.userID!] ?? StrRes.offline;
        },
      );
    } catch (_) {
  ...
  void toCall() {
    IMViews.openIMCallSheet(userInfo.value.showName, (mediaType) {
      imLogic.call(
