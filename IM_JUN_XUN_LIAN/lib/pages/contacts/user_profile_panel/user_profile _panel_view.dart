...
class UserProfilePanelPage extends StatelessWidget {
  ...
  /// 是否显示底部按钮组（音视频通话、发消息）
  bool _shouldShowButtonGroup() {
    // 是自己，不显示
    if (logic.isMyself) return false;
    
    // 是群成员页面且开启了"不允许添加群成员为好友"，无论是否是好友都不显示
    if (logic.isGroupMemberPage && logic.notAllowAddGroupMemberFriend.value) {
      return false;
    }
    
    // 其他情况：是好友或允许非好友发消息，显示
    return logic.isFriendship || logic.allowSendMsgNotFriend;
  }
          ...
          body: SizedBox(
            ...
            child: Stack(
              children: [
                ...
                // 1. 不是自己
                // 2. 是好友，或者允许非好友发消息
                // 3. 如果是群成员页面且开启了"不允许添加群成员为好友"且不是好友，则隐藏
                if (_shouldShowButtonGroup()) _buildButtonGroup(),
              ],
            ),
          ),
        ));
