...
class GroupManageLogic extends GetxController {
  ...
  void toggleMemberProfiles() async {
    final newStatus = !allowLookProfiles ? 1 : 0;
    ...
    groupInfo.update((val) {
      val?.lookMemberInfo = newStatus;
    });
  }

  /// 不允许添加群成员为好友 0：关闭，1：打开
  void toggleAddMemberToFriend() async {
    final newStatus = !allowAddFriend ? 1 : 0;
    await LoadingView.singleton.wrap(
      asyncFunction: () =>
    ...
    // 更新本地状态
    groupInfo.update((val) {
      val?.applyMemberFriend = newStatus;
    });
  }
