...
class ChatLogic extends GetxController {
  ...
  int _messageSortValue(Message message) =>
      message.sendTime ?? message.createTime ?? 0;

  int _compareMessage(Message a, Message b) {
    final timeCompare = _messageSortValue(a).compareTo(_messageSortValue(b));
    if (timeCompare != 0) return timeCompare;

    final createTimeCompare = (a.createTime ?? 0).compareTo(b.createTime ?? 0);
    if (createTimeCompare != 0) return createTimeCompare;

    return (a.clientMsgID ?? '').compareTo(b.clientMsgID ?? '');
  }

  bool _containsMessage(List<Message> list, Message message) =>
      list.any((e) => e.clientMsgID == message.clientMsgID);

  void _insertMessageSorted(List<Message> list, Message message) {
    if (_containsMessage(list, message)) return;

    final index =
        list.indexWhere((element) => _compareMessage(message, element) < 0);
    if (index < 0) {
      list.add(message);
    } else {
  ...
  void _mergeMessagesSorted(Iterable<Message> messages) {
    final sorted = messages.toList()..sort(_compareMessage);
    for (final message in sorted) {
      _insertMessageSorted(messageList, message);
    }
  }

  void _appendMediaMessage(Message message) {
    if ((message.contentType == MessageType.picture ||
            message.contentType == MessageType.video) &&
        !mediaMessages.contains(message)) {
  ...
  void onInit() {
    ...
    imLogic.onRecvNewMessage = (Message message) {
      ...
      if (isCurrentChat(message)) {
        ...
        if (message.contentType == MessageType.typing) {
          ...
          if (!_containsMessage(messageList, message) &&
              !_containsMessage(scrollingCacheMessageList, message)) {
            _isReceivedMessageWhenSyncing = true;
            _parseAnnouncement(message);
            if (isShowPopMenu.value || scrollController.offset != 0) {
              _insertMessageSorted(scrollingCacheMessageList, message);
            } else {
              _appendMediaMessage(message);
              _insertMessageSorted(messageList, message);
              scrollBottom();
            }
            // ios 退到后台再次唤醒消息乱序
      ...
      try {
        for (var readInfo in list) {
          if (readInfo.userID == userID) {
            final hasMsgIDList = readInfo.msgIDList != null && readInfo.msgIDList!.isNotEmpty;
            for (var e in messageList) {
              if (e.sendID == OpenIM.iMManager.userID && !e.isRead!) {
                if (hasMsgIDList) {
                  // 有具体消息ID列表时，按ID匹配
                  ...
                  // 没有消息ID列表时（markConversationMessageAsRead），标记所有已发消息为已读
                  e.isRead = true;
                  e.hasReadTime = _timestamp;
                }
              }
    ...
    imLogic.onSignalingMessage = (value) {
      if (value.isSingleChat && value.userID == userID ||
        ...
        _insertMessageSorted(messageList, value.message);
        scrollBottom();
      }
    };
  ...
  void sendPicture({required String path}) async {
    try {
      ...
      final imagePath = file?.path ?? path;
      var message =
      ...
      Logger.print('sendPicture error: $e $s');
      IMViews.showToast('图片发送失败，请重试');
    }
  }
  ...
  void _sendSucceeded(Message oldMsg, Message newMsg) {
    ...
    messageList.sort(_compareMessage);
      ...
      if (isSingleChat) {
        ...
        if (null != customType) {
          final hintMessage = (await OpenIM.iMManager.messageManager
            ...
            ..status = 2
            ..isRead = true;
          _insertMessageSorted(messageList, hintMessage);
  ...
  void onTapCamera() async {
    ...
    try {
      ...
      IMViews.showToast('打开相机失败，请检查相机权限');
    }
  }
  ...
  void _handleAssets(AssetEntity? asset) async {
    if (null != asset) {
      ...
      if (sourceFile == null) {
        IMViews.showToast('读取媒体文件失败，请重试');
        return;
      }
  ...
  void call() async {
    ...
    if (isGroupChat) {
      ...
      IMViews.openIMGroupCallSheet(groupID!, (mediaType) async {
        if (null != groupInfo) {
          ...
          if (list is List<GroupMembersInfo>) {
            ...
            imLogic.call(
              ...
              callType: mediaType == 'audio' ? CallType.audio : CallType.video,
              groupID: groupID,
      ...
      IMViews.openIMCallSheet(nickname.value, (mediaType) {
        imLogic.call(
          callObj: CallObj.single,
          callType: mediaType == 'audio' ? CallType.audio : CallType.video,
          inviteeUserIDList: [if (isSingleChat) userID!],
  ...
  void _configUserStatusChanged(UserStatusInfo? status) {
    if (status != null) {
      final platformIDs = status.platformIDs ?? const <int>[];
      final isOnline = platformIDs.isNotEmpty || status.status == 1;
      onlineStatus.value = isOnline;
      onlineStatusDesc.value = isOnline ? StrRes.online : StrRes.offline;
    }
  }
  ...
  void onPopMenuShowChanged(show) {
    ...
    if (!show && scrollingCacheMessageList.isNotEmpty) {
      _mergeMessagesSorted(scrollingCacheMessageList);
