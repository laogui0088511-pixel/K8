...
abstract class SignalView extends StatefulWidget {
  const SignalView({
    ...
    this.disableLocalVideoPublish = false,
  ...
  final bool disableLocalVideoPublish;
...
abstract class SignalState<T extends SignalView> extends State<T> {
  ...
  _onStateDidUpdate(CallEvent event) {
    ...
    if (event.state == CallState.call ||
      ...
      if (!callStateSubject.isClosed) callStateSubject.add(event.state);
    }

    if (event.state == CallState.beRejected ||
        event.state == CallState.beCanceled) {
      // 如果挂断用户非当前聊天用户，或不是单聊用户 不做处理
      if (event.data.userID != widget.userID ||
          event.data.invitation!.sessionType != 1) {
        return;
      }
      widget.onClose?.call();
  ...
  onParticipantConnected() {
    ...
    if (!callStateSubject.isClosed) callStateSubject.add(CallState.calling);
  ...
  onTapPickup() async {
    ...
    if (!callStateSubject.isClosed) callStateSubject.add(CallState.connecting);
    try {
      certificate = await widget.onTapPickup!.call();
    } catch (e) {
      Logger.print('onTapPickup error: $e');
      widget.onClose?.call();
      return;
    }
    if (!mounted) return;
