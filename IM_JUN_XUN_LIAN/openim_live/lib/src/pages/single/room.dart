...
class SingleRoomView extends SignalView {
  const SingleRoomView({
    ...
    super.disableLocalVideoPublish,
...
class _SingleRoomViewState extends SignalState<SingleRoomView> {
  ...
  Room? _room;
  Timer? _participantDisconnectedTimer;

  @override
  void dispose() {
    // always dispose listener
    (() async {
      _participantDisconnectedTimer?.cancel();
  ...
  void _setUpListeners() => _listener!
    ...
    ..on<ParticipantConnectedEvent>((_) {
      _participantDisconnectedTimer?.cancel();
      _participantDisconnectedTimer = null;
      onParticipantConnected();
    })
    ..on<ParticipantDisconnectedEvent>((_) {
      _participantDisconnectedTimer?.cancel();
      _participantDisconnectedTimer = Timer(const Duration(seconds: 8), () {
        if (!mounted) return;
        if (_room?.participants.isEmpty ?? true) {
  ...
  void _publish() async {
    ...
    try {
      var enabled =
          widget.callType == CallType.video && !widget.disableLocalVideoPublish;
      if (enabled) {
        final devices = await Hardware.instance.enumerateDevices();
        final hasVideoInput = devices.any((d) => d.kind == 'videoinput');
        if (!hasVideoInput) {
          enabled = false;
          IMViews.showToast('该设备没有可用摄像头，当前无法发送本地视频');
        }
      }
