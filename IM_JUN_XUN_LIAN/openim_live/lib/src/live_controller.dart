...
mixin OpenIMLive {
  ...
  _signalingListener() => _stream.listen(
        (event) async {
          ...
          if (event.state == CallState.beCalled) {
            ...
            final disableLocalVideoPublish = callType == CallType.video
                ? !await _hasVideoInputDevice()
                : false;
            ...
            OpenIMLiveClient().start(
              ...
              callType: callType,
              disableLocalVideoPublish: disableLocalVideoPublish,
              callObj: callObj,
  ...
  call({
    ...
    final disableLocalVideoPublish =
        callType == CallType.video ? !await _hasVideoInputDevice() : false;
    if (disableLocalVideoPublish) {
      IMViews.showToast('该设备没有可用摄像头，将以视频通话加入（无本地视频画面）');
    }
  ...
  Future<bool> _hasVideoInputDevice() async {
    try {
